...
const details = () => ({
  id: 'Tdarr_Plugin_MC93_Migz5ConvertAudio_DRC',
  Stage: 'Pre-processing',
  Name: 'Migz Convert Audio Streams DRC',
  Type: 'Audio',
  Operation: 'Transcode',
  Description: 'This plugin can convert any 2.0 audio track/s to AAC and can create downmixed audio tracks. This plugin also applies DRC to the down mixed stereo track and creates a 6 channel EAC3 audio track if one does not exists. \n\n',
  Version: '2.4',
...
const plugin = (file, librarySettings, inputs, otherArguments) => {
  ...
  for (let i = 0; i < file.ffProbeData.streams.length; i++) {
    ...
    if (file.ffProbeData.streams[i].codec_type.toLowerCase() === 'audio') {
      // Catch error here in case user left inputs.downmix empty.
      try {
        ...
        if (inputs.downmix === true) {
          ...
          if (
            ...
            ffmpegCommandInsert += `-map 0:${i} -c:a:${audioIdx} eac3 -b:a 640k -ac 6 -metadata:s:a:${audioIdx} title= `;
          ...
          if (
            ...
            ffmpegCommandInsert += `-map 0:${i} -c:a:${audioIdx} aac -b:a:${audioIdx} 256k -ac 2 `
  + `-filter:a:${audioIdx} "dynaudnorm" `
  + `-metadata:s:a:${audioIdx} title= `;
      ...
      // Catch error here in case user left inputs.aac_stereo empty.
      try {
        // Check if inputs.aac_stereo is set to true.
        if (inputs.aac_stereo === true) {
          // Check if codec_name for stream is NOT aac AND check if channel amount is 2.
          if (
